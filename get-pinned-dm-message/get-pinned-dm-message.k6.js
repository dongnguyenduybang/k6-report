...
export default function (data) {
  ...
  const testCases = [
    ...
    {
      number: 3,
      title: 'should return errors ["userId should not be null"] when body {"userId":null}',
      payload: {"userId":null},
      expectedErrors: ["userId should not be null"]
    },
  ...
  testCases.forEach(test => {
    group(`Test case #${test.number}: ${test.title}`, () => {
      ...
      const params = Object.keys(resolvedData).map(key => `${key}=${encodeURIComponent(resolvedData[key])}`).join('&');
        const url = 'https://api-dev.ziichat.dev/MessageView/GetPinnedDMMessage' + (params ? `?${params}` : '');
