...
export default function (data) {
  ...
  const testCases = [
    ...
    {
      number: 2,
      title: 'should return errors ["Could not resolve permission type"] when body {"channelId":"{{channelId}}"}',
    ...
    {
      number: 4,
      title: 'should return errors ["Could not resolve permission type"] when body {"workspaceId":null,"channelId":"{{channelId}}"}',
    ...
    {
      number: 7,
      title: 'should return errors ["Unsupported permission type"] when body {"workspaceId":"0"}',
  ...
  testCases.forEach(test => {
    group(`Test case #${test.number}: ${test.title}`, () => {
      ...
      const url = 'https://api-dev.ziichat.dev/MessageView/ListMessageFragments' + (params ? `?${params}` : '');
